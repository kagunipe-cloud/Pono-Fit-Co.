...
export async function runKisiAccessAudit(
  ...
  if (options?.member_ids?.length) {
    const placeholders = options.member_ids.map(() => "?").join(",");
    memberRows = db
      .prepare(
      ...
      .all(...options.member_ids) as MemberAuditBase[];
  } else {
    memberRows = db
      .prepare(
        `SELECT member_id, email, first_name, last_name, kisi_id, waiver_signed_at,
...
export function countMembersForKisiAudit(db: ReturnType<typeof getDb>): number {
  const row = db
