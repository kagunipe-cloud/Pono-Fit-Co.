...
export async function GET(request: NextRequest) {
  try {
    ...
    let rows = db.prepare("SELECT * FROM discounts ORDER BY code ASC").all() as {
      id: number;
      code: string;
      percent_off: number;
    ...
    if (rows.length === 0) {
      ...
      for (const d of defaults) {
        try {
          db.prepare("INSERT INTO discounts (code, percent_off, description, scope, applies_to_renewals) VALUES (?, ?, ?, 'cart', 0)").run(
            d.code,
            d.percent_off,
            d.description
          );
      ...
      rows = db.prepare("SELECT * FROM discounts ORDER BY code ASC").all() as typeof rows;
    }
    db.close();
...
export async function POST(request: NextRequest) {
  try {
    ...
    const appliesToRenewals = body.applies_to_renewals === true || body.applies_to_renewals === 1 ? 1 : 0;
    ...
    const result = db.prepare(
      "INSERT INTO discounts (code, percent_off, description, scope, applies_to_renewals) VALUES (?, ?, ?, ?, ?)"
    ).run(code, percentOff, description, scope, appliesToRenewals);
