...
/** GET: current day pass ledger balance. Admin only. */
export async function GET(
  ...
  try {
    ...
    console.error(err);
    return NextResponse.json({ error: "Failed to fetch day pass credits" }, { status: 500 });
  }
}

/**
 * POST: grant or remove day pass credits. Body:
...
export async function POST(
  ...
  try {
    ...
    if (action === "remove") {
      if (balance <= 0) {
        db.close();
        return NextResponse.json({ error: "No day pass credits to remove." }, { status: 400 });
      }
    ...
    const newBalance = getMemberDayPassLedgerBalance(db, memberId);
    db.close();
    ...
    console.error(err);
    return NextResponse.json({ error: "Failed to update day pass credits" }, { status: 500 });
  }
}
