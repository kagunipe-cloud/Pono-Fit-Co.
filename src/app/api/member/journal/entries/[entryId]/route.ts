...
export async function PATCH(
  ...
  try {
    ...
    const entrySelect = hasDisplayUnits
      ? "SELECT e.id, e.quantity, e.measurement"
      : "SELECT e.id";
    const entry = db.prepare(
      `${entrySelect} FROM journal_meal_entries e
    ...
    ).get(entryId, memberId) as { id: number; quantity?: number | null; measurement?: string | null } | undefined;
    if (!entry) {
      db.close();
      return NextResponse.json({ error: "Not found" }, { status: 404 });
    }
    if (hasDisplayUnits) {
      const hasMeasurement = entry.measurement != null && String(entry.measurement).trim() !== "";
      if (hasMeasurement) {
        db.prepare("UPDATE journal_meal_entries SET amount = ?, quantity = ? WHERE id = ?").run(amount, amount, entryId);
      } else {
