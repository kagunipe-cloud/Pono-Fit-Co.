...
export async function GET() {
  try {
    ...
    const row = db.prepare(
      "SELECT calories_goal, protein_pct, fat_pct, carbs_pct FROM member_macro_goals WHERE member_id = ?"
    ).get(memberId) as { calories_goal: number | null; protein_pct: number | null; fat_pct: number | null; carbs_pct: number | null } | undefined;
    db.close();
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    const db = getDb();
    ensureJournalTables(db);
    const existing = db.prepare("SELECT member_id FROM member_macro_goals WHERE member_id = ?").get(memberId);
    if (!existing) {
      db.prepare(
      ...
      const row = db.prepare("SELECT calories_goal, protein_pct, fat_pct, carbs_pct FROM member_macro_goals WHERE member_id = ?").get(memberId) as { calories_goal: number | null; protein_pct: number | null; fat_pct: number | null; carbs_pct: number | null };
      const cal = calories_goal !== undefined ? calories_goal : row.calories_goal;
      const p = protein_pct !== undefined ? protein_pct : row.protein_pct;
      const f = fat_pct !== undefined ? fat_pct : row.fat_pct;
      const c = carbs_pct !== undefined ? carbs_pct : row.carbs_pct;
      db.prepare(
