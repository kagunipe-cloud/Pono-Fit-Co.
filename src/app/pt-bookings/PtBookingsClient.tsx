...
export function PtBookingsClient() {
  ...
  const fetchData = useCallback(async (query: string) => {
    ...
    try {
      const url = query
        ? `/api/data/pt-bookings-unified?q=${encodeURIComponent(query)}`
        : "/api/data/pt-bookings-unified";
  ...
  return (
    <div className="bg-white rounded-xl border border-stone-200 shadow-sm overflow-hidden">
      <div className="p-4 border-b border-stone-100 flex flex-wrap items-center gap-3">
        <input
          type="search"
          placeholder="Search PT bookings..."
          value={search}
      ...
      {loading ? (
        ...
        <div className="p-12 text-center text-stone-500">No PT bookings found.</div>
      ) : (
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="bg-stone-50 text-stone-500 text-sm font-medium">
                ...
                <th className="py-3 px-4">Member / Guest</th>
